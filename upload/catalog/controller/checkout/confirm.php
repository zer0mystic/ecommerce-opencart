...
class Confirm extends \Opencart\System\Engine\Controller {
	...
	/**
 * Confirm Order (AJAX - dipanggil setelah Midtrans sukses)
 *
 * @return void
 */
	public function confirmOrder(): void {
		...
		if (!isset($this->session->data['payment_method'])) {
			$json['error'] = 'Payment method belum dipilih.';
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
			return;
		}

		// Validasi shipping jika diperlukan
		if ($this->cart->hasShipping()) {
			if (!isset($this->session->data['shipping_address']['address_id'])) {
				$json['error'] = 'Shipping address belum diisi.';
				$this->response->addHeader('Content-Type: application/json');
				$this->response->setOutput(json_encode($json));
				return;
			}
			...
			if (!isset($this->session->data['shipping_method'])) {
				$json['error'] = 'Shipping method belum dipilih.';
				$this->response->addHeader('Content-Type: application/json');
				$this->response->setOutput(json_encode($json));
				return;
			}
		...
		if ($this->config->get('config_checkout_payment_address') && isset($this->session->data['payment_address'])) {
			...
			$order_data['payment_address_id']    = 0;
			$order_data['payment_firstname']     = $this->session->data['customer']['firstname'];
			$order_data['payment_lastname']      = $this->session->data['customer']['lastname'];
			$order_data['payment_company']       = '';
			$order_data['payment_address_1']     = '';
			$order_data['payment_address_2']     = '';
			$order_data['payment_city']          = '';
		...
		unset($this->session->data['midtrans_result']);
		unset($this->session->data['midtrans_order_ref']);
